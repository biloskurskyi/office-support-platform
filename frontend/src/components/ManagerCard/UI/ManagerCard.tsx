...
const ManagerCard: React.FC<ManagerCardProps> = ({ manager }) => {
  ...
  const handleToggleActive = async () => {
    try {
      ...
      alert(
        `Статус менеджера змінено на ${response.data.is_active ? 'активний' : 'неактивний'}.`
      );
    } catch (error) {
  ...
  return (
    <div>
      <p>
        <strong>Прізвище:</strong> {manager.surname}
      </p>
      <p>
        <strong>Ім'я:</strong> {manager.name}
      </p>
      <p>
        <strong>Електрона адреса:</strong> {manager.email}
      </p>
      <p>
        <strong>Інформація:</strong> {manager.info}
      </p>
      <Typography
        variant="body2"
