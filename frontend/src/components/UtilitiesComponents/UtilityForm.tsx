...
const UtilityForm: React.FC<UtilityFormProps> = ({
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://localhost:8765/api/utilities/types/${id}/`, {
    ...
    <>
      <form onSubmit={handleSubmit}>
        <Grid container spacing={2}>
          ...
          <Grid item xs={12}>
            ...
            >
              ...
              >
                ...
                {utilities.map(
                  (manager: { id: number; utilities_type: string }) => (
                    <MenuItem
                      key={manager.id}
                      value={manager.id}
                      sx={{
          ...
          <Grid item xs={12}>
            <FormHelperText sx={{ fontSize: '0.75rem', color: 'gray' }}>
              * при виборі Збору відходів поле з показником лічильника власноруч
              збільшиться на 1
            </FormHelperText>
          </Grid>
          <CustomTextField
        ...
        <ul>
          ...
          <li>
            Виберіть дату реєстрації. В один місяць не може бути зареєстровано
            більше ніж 1 унікальна послуга певного типу для офісу.
          </li>
          <li>
            Обов’язково заповніть показники лічильника (за винятком послуг збору
            відходів).
          </li>
          <li>
            Показник лічильника не може бути меншим за показник попереднього
            місяця та більшим за наступний!
          </li>
        </ul>

        <SubmitButton
          text="Створити запис про комунальну послугу"
