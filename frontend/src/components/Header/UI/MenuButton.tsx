          ...
          localStorage.getItem('user_type') === '2' ? (
            offices.length > 0 ? (
              offices.map((office) => (
                <MenuItem
                  key={office.id}
                  onClick={(o) => handleOfficeClick(o, office.phone_number)}
                >
                  {isSmallScreen ? (
                    <>
            ...
            companies.map((company) => (
              <MenuItem
                key={company.id}
                onClick={(e) => handleCompanyClick(e, company.legal_name)}
              >
                {company.legal_name}
              </MenuItem>
            ))
              ...
              <MenuItem onClick={handleMenuClose}>
                {isSmallScreen ? (
                  <>
                    Створіть компанію,
                    <br />
                    щоб почати управляти вже зараз
                  </>
                ) : (
            ...
            >
              {isSmallScreen ? (
                <>
                  Увійдіть,
                  <br />
                  щоб побачити перелік можливостей
                </>
              ) : (
        ...
        {localStorage.getItem('user_type') === '1' ? (
          [
            <Link
              ...
              style={{ textDecoration: 'none', color: 'inherit' }}
            >
              <MenuItem key="managers" onClick={handleSubmenuItemClick}>
                Менеджери
              </MenuItem>
            </Link>,
            <Link
            ...
            <Link
              to={`/office-overview-for-utilities/${
              ...
              style={{ textDecoration: 'none', color: 'inherit' }}
            >
              <MenuItem key="communal" onClick={handleSubmenuItemClick}>
                Комунальні послуги
              </MenuItem>
            </Link>,
            <Link
              to={`/office-overview-for-orders/${
                Array.isArray(companies)
                  ? companies.find((c) => c.legal_name === selectedCompany)?.id
                  : ''
              }`}
              style={{ textDecoration: 'none', color: 'inherit' }}
            >
              <MenuItem key="orders" onClick={handleSubmenuItemClick}>
                Замовлення
              </MenuItem>
            </Link>,
            <Link
              ...
              style={{ textDecoration: 'none', color: 'inherit' }}
            >
              <MenuItem key="providers" onClick={handleSubmenuItemClick}>
                Постачальники
              </MenuItem>
            </Link>,
            <Link
