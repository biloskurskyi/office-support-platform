...
const CreateCompanyForm = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await axios.post('http://localhost:8765/api/company/', formData, {
                headers: {
            ...
            if (error.response) {
                if (error.response.status === 400) {
                    ...
                    setErrorMessage('Невірно введені данні, ' +
    ...
    return (
        ...
        >
            <FormPaper title="Створення компанії">
                ...
                {successMessage && (
                    <Typography color="success.main" sx={{marginTop: '10px', color: 'green'}}>
                        {successMessage}
                    </Typography>
                )}
            </FormPaper>
